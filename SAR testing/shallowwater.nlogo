patches-own [zpos delta-z ]
globals [last-mouse k data time]

extensions[ py]

to setup
  ask patches [set zpos 0 set delta-z 0]
  color-patches
...
to create-wave
  if mouse-down? [
    let current-mouse patch  mouse-xcor mouse-ycor
    if current-mouse != last-mouse [ask current-mouse [set zpos strength] ]
    set last-mouse current-mouse
  ]
end
...
to color-patches
  let max-water 10
  ask patches [set pcolor scale-color blue zpos (-1 * max-water) max-water]
   ask turtles [set label precision zpos 1]  ; show buoy
end

to compute-delta-z    ;Turtle procedure
  set k (1 - (.01 * surface-tension))        ;k determines the degree to which neighbor-turtles'
  set delta-z (delta-z + (k * ((sum [zpos] of neighbors4) - ((count neighbors4) * zpos))))
end

to update-z
   set zpos ((zpos + delta-z)  * friction)
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 1 [ask patch 0 0 [plot zpos]]"
