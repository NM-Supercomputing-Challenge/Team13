...
to setup
  ...
  ask patch 0 0 [sprout-microphones 1 [set shape "circle"
    set size 4 set color red
    set label precision zpos 1
  ]]

  color-patches
  create-speakers 1 [setxy 25 25 set shape "square 2" set size 4  set color green]
  reset-ticks
  clear-all-plots
...
to oscillate
  ...
  ask speakers [set zpos (amplitude * sin ((ticks + phase) * frequency * 2 * pi)) + (15 * sin ((ticks) * 8 * 2 * pi))]
end
...
"pen-1" 1.0 0 -7500403 true "" "plot [zpos] of one-of speakers"
