...
to color-patches
  let max-water 10
  ask patches [
    set pcolor scale-color blue zpos (-1 * max-water) max-water
...
to listen
  ...
  ;ask speakers [if zpos > cutoff and speakListening = True [calculate]]
  ;ask speakers [if speakListening = true [set zpos 0]]


  ;ask speakers [if zpos < speakerlastzpos and speakListening = true [set speacListening false]

  ;ask speakers [set speakerlastzpos zpos]

  ;ask speakers [set slist lput sSignal slist]]
  ;ask microphones [set mlist lput zpos mlist]

  ;ask microphones [if abs(zpos) < 1 [set mlist lput zpos mlist]]

  ;ask microphones [if micListening = true and abs(zpos) > 0.1 [print ticks set micListening false]]

  ;ask microphones [if zpos > 0.1 and micListening = True [return]]



  ;ask microphones [set cutoff (2 * max mlist)]
  ;print sSignal
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    ...
    <go>go</go>
    <metric>mag</metric>
    <steppedValueSet variable="surface-tension" first="90" step="0.25" last="99"/>
    <steppedValueSet variable="sustain" first="0.99" step="5.0E-4" last="0.995"/>
    <enumeratedValueSet variable="dis">
      <value value="20"/>
      <value value="40"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="surface only" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mag</metric>
    <steppedValueSet variable="surface-tension" first="97.5" step="0.1" last="99.5"/>
  ...
  <experiment name="sustain" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mag</metric>
    ...
    <steppedValueSet variable="sustain" first="0.98" step="0.001" last="1"/>
    <enumeratedValueSet variable="dis">
      <value value="20"/>
      <value value="40"/>
    </enumeratedValueSet>
